...
import { getOJTsAdmin, UpdateOJTStatus, updateOJTStatus } from '@/lib/api';
...
function RouteComponent() {
  const { isPending, data: ojts } = useQuery({
    queryKey: ['ojtsAdmin'],
  ...
  const onUpdateStatus = (data: UpdateOJTStatus) => {
    mutate(data, {
      onSuccess: ({ message }) => {
        ...
        setEditingRow(null);
        queryClient.invalidateQueries({ queryKey: ['ojtsAdmin'] });
      },
    });
  };
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>OJT Status</TableHead>
